...
pen_anti_model <- ggplot(combined_tidy_models, aes(x = estimate, y = term, color = factor(looks, levels = c("target looks", "Competitor looks")))) +
  geom_point() +
  geom_errorbarh(aes(xmin = estimate - 1.96 * std.error, xmax = estimate + 1.96 * std.error), height = 0) +
  labs(title = "", x = "", y = "", color = "Looks to Visual Stimuli") +
  theme_minimal()+
  geom_vline(xintercept = 0, color = "#007bc0")+
  coord_flip()+
  scale_color_manual(
    values = c("target_looks" = "#c41230", "competitor_looks" = "#007bc0", "distractor_1" = "#000000", "distractor_2" = "#A7A7A9"),
    labels = c("Target", "Competitor\n(Base-line in model)", "Distractor 1", "Distractor 2\n(not in model)")
  )
  xlim(-2.7, 1) 
  pen_anti_model
  theme(
  ...
  geom_text(aes(y = 1, x = -3.5, label = "Terms"), color = "black", hjust = 1, size = 5, angle = 0, show.legend = FALSE) +
  geom_text(aes(y = row_number, x = -1.2, label = Significance), hjust = 0, vjust = .5, angle = 90, color = "black", show.legend = FALSE) +
  # Add dummy points to include all looks in the legend
  geom_point(data = dummy_data, aes(x = estimate, y = row_number, color = factor(looks, levels = c("target", "Base_looks", "distractor_1", "distractor_2"))), show.legend = TRUE) +
  # Make the dummy points invisible in the plot
  geom_point(data = dummy_data, aes(x = estimate, y = row_number, color = factor(looks, levels = c("target", "Base_looks", "distractor_1", "distractor_2"))), alpha = 0, show.legend = FALSE) +
  annotate("rect", xmin = -4, xmax = -1.3, ymin = -2.2, ymax = 10.9, fill = "white", color = "grey") +
...
#this is take data just to get the legend to work
dummy_data <- data.frame(
  estimate = c(-3, -3, -3, -3),  # Use a value outside the actual data range
  row_number = c(-1, -1, -1, -1),  # Use a value outside the actual data range
  looks = c("Base_looks", "distractor_1", "target", "distractor_2")
)

# Create the main plot
pen_anti_model <- ggplot(tidy_model, aes(x = estimate, y = row_number, color = factor(looks, levels = c("target", "Base_looks", "distractor_1", "distractor_2")))) +
  geom_point() +
  geom_errorbarh(aes(xmin = estimate - 1.96 * std.error, xmax = estimate + 1.96 * std.error), height = 0) +
  labs(title = "", x = "", y = "", color = "Looks to Visual Stimuli") +
  theme_minimal() +
  ...
  scale_color_manual(
    values = c("target" = "#c41230", "Base_looks" = "#007bc0", "distractor_1" = "#000000", "distractor_2" = "#A7A7A9"),
    labels = c("Target", "Competitor\n(Base-line in model)", "Distractor 1", "Distractor 2\n(not in model)")
  ) +
  xlim(-2.7, 1) +
  coord_flip() +
...
analysis_3_plot <- ggplot(tidy_combined, aes(x = estimate, y = term, color = model)) +
  ...
  scale_x_continuous(limits = c(-1.5, 1.75))
# Print the plot
analysis_3_plot 
