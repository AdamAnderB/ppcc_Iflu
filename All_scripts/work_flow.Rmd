...
```{r}
comp_tar_longer_example_fixations<-comp_targ%>%
  pivot_longer(cols=c(target_looks:dist2_looks),names_to = "names",values_to="values")%>%
  mutate(syllable=as.factor(syllable),
         stress_play=as.factor(stress_play))%>%
  filter(names=="target_looks"|names=="competitor_looks"|names=="dist1_looks")

filtered_data_proof <- comp_tar_longer_example_fixations %>%
  filter(x_pred_normalised <= 0.5 & x_pred_normalised >= -0.5 & 
         y_pred_normalised <= 0.5 & y_pred_normalised >= -0.5) %>%
  filter(!(x_pred_normalised > -0.01 & x_pred_normalised < 0.01 | 
           y_pred_normalised > -0.01 & y_pred_normalised < 0.01))

# Create the plot
proof <- ggplot(filtered_data_proof, aes(x = x_pred_normalised, y = y_pred_normalised,color=syllable)) +
  geom_point(size = 1, alpha = 1)



proof <- ggplot(filtered_data_proof, aes(x = x_pred_normalised, y = y_pred_normalised, group = interaction(as.factor(time_rounded),stress_play))) +
  geom_pointdensity(adjust = 5, size = 1) +
  scale_color_gradientn(colors = c("blue", "yellow", "red")) +  # Use gradient colors for density
  theme_minimal()+facet_grid(stress_play~time_rounded,scales="free")
proof
```
...
circular_plot_polygon <- ggplot(acoustic_data_longer3, aes(x = angle, y = scaled_value,color=stress_play, group = interaction(syllable, base, stress_play))) +
  ...
  coord_straightpolar(start = 0) +
  theme_minimal() +
  scale_fill_manual(values = c("Antepenultimate" = "#C41230", "Penultimate" = "#000000")) +
  scale_color_manual(values = c("Antepenultimate" = "#C41230", "Penultimate" = "#000000")) +
  theme(
...
raw_acoustics<-acoustic_data_longer %>%
  ...
  geom_smooth(aes(x = syllable,  y = values, color = stress_play, 
                  group = interaction(stress_play, type),SE = FALSE), method = "lm",size=1) +
  geom_smooth(aes(x = syllable,  y = values, 
                  group = interaction(stress_play, type),SE = FALSE), color = "white",method = "lm", se = FALSE,size=1,linetype="dotted") +
