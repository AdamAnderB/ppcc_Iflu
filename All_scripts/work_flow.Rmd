...
#s_exp1_data_all <- read.csv(file.path(target_dir, "s_exp1_data_all.csv"))
...
lextale_agg<-lextale_tidy%>%
  ...
  select(-c(dprime, c))
densityplot(lextale_agg$lextale_score)
...
italian_lextale_tidy<-italian_lextale%>%
  select(participant.private.id,screen.name,zone.type,reaction.time,spreadsheet.row,correct,response)%>%
  filter(zone.type=="response_keyboard")%>%
...
italian_lextale_agg<-italian_lextale_tidy%>%
  ...
  mutate(
    ...
    c = dprime(hit_count, 
                        ...
                        f_a_count + c_r_count)$c)%>%
  select(participant.private.id, dprime, c)%>%
  mutate(italian_lextale_score=dprime)%>%
  select(-c(dprime, c))

densityplot(italian_lextale_agg$italian_lextale_score)
...
italian_lextale_cb <- italian_lextale_tidy %>%
  select(-reaction.time) %>%
  pivot_wider(names_from = spreadsheet.row, values_from = correct) %>%
  select(-participant.private.id)%>%
  mutate(across(everything(), ~replace_na(., 0)))

italian_lextale_cb_a<-cronbach.alpha(italian_lextale_cb)$alpha
...
autism_cb<-autism_tidy%>%
  pivot_wider(names_from = question.key,values_from = response)%>%
  select(-participant.private.id)

autism_cb_a<-cronbach.alpha(autism_cb)$alpha
autism_cb_a

autism_cleaned%>%ggplot(aes(x=autism_score))+geom_histogram()
```
#cronbach alphas for each individual difference task
```{r}
cronbach_list <- list(
  battery_duration = battery_cb_dur_a,
  battery_formants = battery_cb_formants_a,
  battery_risetime = battery_cb_risetime_a,
  battery_pitch = battery_cb_pitch_a,
  digit_span = digit_cb_a,
  lextale = lextale_cb_a,
  italian_lextale = italian_lextale_cb_a,
  autism = autism_cb_a
)

cronbachs_alpha_df <- data.frame(
  Dataset = names(cronbach_list),
  Alpha = sapply(cronbach_list, function(x) round(x, 2))
)
hist(cronbachs_alpha_df$Alpha)

write_csv(cronbachs_alpha_df, file.path(target_dir, "cronbach_alpha.csv"))
```
...
s_exp1_data_all <- s_exp1_et_cleaned %>%
  ...
  mutate(time_rounded=time_rounded-1200)%>% #offset for item experiment timing
...
plot1<-ets_exp1_all_agg%>%
  filter(stress_play == "Penultimate") %>%
  ggplot(aes()) +
  geom_line(data=summarized_data_penn,aes(x = time_rounded, y = values,group = interaction(names,participant.private.id,stress_play),color=names),alpha=.8,size=.05)+
  geom_text(data=df_1,aes(label=labels,x = time_rounded, y = values),
            family = "Arial", size = 4, fontface = "bold")+
  geom_point(aes(x = time_rounded, y = values, color = names)) +
  geom_smooth(aes(x = time_rounded, y = values, color = names),se=FALSE)+
...
##commented out ones were run originally but take a long time and did not converge so we have commented them out to make rerunning our analysis more time effective.
...
comp_tar_model2<-glmer(values~names*stress_play*syllable+(1|participant.private.id)+(1|word_play),data=comp_tar_longer,family = "binomial")
comp_tar_model3<-glmer(values~stress_play+names*syllable+(1|participant.private.id)+(1|word_play),data=comp_tar_longer,family = "binomial")
comp_tar_model4<-glmer(values~names+stress_play*syllable+(1|participant.private.id)+(1|word_play),data=comp_tar_longer,family = "binomial")

#summary(comp_tar_model)
#summary(comp_tar_model1)
summary(comp_tar_model2)
summary(comp_tar_model3)
summary(comp_tar_model4)

#anova(comp_tar_model,comp_tar_model1)
#anova(comp_tar_model1,comp_tar_model2)
anova(comp_tar_model2,comp_tar_model3)
anova(comp_tar_model3,comp_tar_model4)
...
# Function to run models and return models + AIC/BIC values
run_models <- function(data, target_type) {
  
  # Filter data for target_looks or competitor_looks
  filtered_data <- data %>%
    filter(names == target_type) %>%
    filter(syllable != 0)
  
  # Full model with random intercepts for participants and words
  full_model <- glmer(values ~ stress_play * syllable + 
                      (stress_play| participant.private.id) + (stress_play| word_play), 
                      data = filtered_data, family = "binomial")
  
  # Reduced models
  reduced_model_1 <- glmer(values ~ stress_play * syllable + 
                      (stress_play|| participant.private.id) + (stress_play|| word_play), 
                      data = filtered_data, family = "binomial")

  reduced_model_2 <- glmer(values ~ stress_play * syllable + 
                           (1 | participant.private.id) + (1 | word_play), 
                           data = filtered_data, family = "binomial")

  # Perform ANOVA comparisons
  anova_results <- anova(full_model, reduced_model_1, reduced_model_2)
  
  # Perform AIC and BIC calculations
  aic_values <- c(full = AIC(full_model), reduced_1 = AIC(reduced_model_1), reduced_2 = AIC(reduced_model_2))
  bic_values <- c(full = BIC(full_model), reduced_1 = BIC(reduced_model_1), reduced_2 = BIC(reduced_model_2))
  
  # Return models and AIC/BIC values
  list(full_model = full_model, reduced_model_1 = reduced_model_1, reduced_model_2 = reduced_model_2, 
       anova_results = anova_results, aic_values = aic_values, bic_values = bic_values)
}

# Run models for target_looks and competitor_looks
target_results <- run_models(comp_tar_longer, "target_looks")
competitor_results <- run_models(comp_tar_longer, "competitor_looks")
...
# Function to select the best model based on AIC and convergence check
select_best_model <- function(models) {
  
  # Check if models converged before selecting based on AIC
  if (!model_converged(models$full_model)) {
    message("Full model did not converge. Trying reduced models.")
  } else {
    message("Full model converged.")
  }
  
  if (!model_converged(models$reduced_model_1)) {
    message("Reduced model 1 did not converge. Trying reduced model 2.")
  } else {
    message("Reduced model 1 converged.")
  }
  
  if (!model_converged(models$reduced_model_2)) {
    message("Reduced model 2 did not converge. This is the last option.")
  } else {
    message("Reduced model 2 converged.")
  }

  # Compare models based on AIC and select the model with the lowest AIC, only if converged
  if (model_converged(models$reduced_model_1) && models$aic_values["reduced_1"] < models$aic_values["full"]) {
    return(models$reduced_model_1)
  } else if (model_converged(models$full_model)) {
    return(models$full_model)
  } else {
    return(models$reduced_model_2)
  }
}

# Select best models for target and competitor
best_target_model <- select_best_model(target_results)
best_competitor_model <- select_best_model(competitor_results)

# Print summary of best models
summary(best_target_model)
summary(best_competitor_model)
```
```{r}

tidy_target_model <- tidy(best_target_model, effects = "fixed") %>%
  mutate(looks = "Target Looks")

tidy_competitor_model <- tidy(best_competitor_model, effects = "fixed") %>%
  mutate(looks = "Competitor Looks")

# Combine the two tidied models
combined_tidy_models <- bind_rows(tidy_target_model, tidy_competitor_model)

# Add other necessary columns and transformations
combined_tidy_models <- combined_tidy_models %>%
  mutate(
    main = ifelse(str_detect(term, ":"), 1, 0),
    syllable = case_when(
      str_detect(term, "syllable1") ~ "1",
      str_detect(term, "syllable2") ~ "1.5",
      str_detect(term, "syllable3") ~ "2",
      TRUE ~ "0"),
    term = str_replace_all(term, c(
      "syllable1" = "Syllable 1", 
      "syllable2" = "Syllable 1.5", 
      "syllable3" = "Syllable 2",
      "stress_play1" ="Stress Ante")),
...
# Combine looks for legend
combined_tidy_models$looks <- factor(combined_tidy_models$looks, levels = c("Target Looks", "Competitor Looks"))
combined_tidy_models <- droplevels(combined_tidy_models)

combined_tidy_models$term <- factor(combined_tidy_models$term, 
                                    levels = c("(Intercept)", 
                                               "Stress Ante", 
                                               "Stress Ante:Syllable 1.5", 
                                               "Stress Ante:Syllable 2", 
                                               "Syllable 1.5", 
                                               "Syllable 2"))
```

```{r}
colors <- c("Target Looks" = "#c41230", "Competitor Looks" = "#007bc0")
#WARNING: combined_tidy_models_legend was created to allow the legend to work. The data is double plotted with the second set of colors with the only purpose of allowing the legend to print correctly. It is just fake filler that is not visable on the viusaliztion so that we can have all for colors in the plot legend
combined_tidy_models_legend <- combined_tidy_models
combined_tidy_models_legend$looks <- factor(combined_tidy_models_legend$looks, 
                                            levels = c("Target Looks", "Competitor Looks"), 
                                            labels = c("Distractor 1", "Distractor 2"))
combined_tidy_models <- droplevels(combined_tidy_models)
combined_tidy_models_legend$term <- factor(combined_tidy_models_legend$term, 
                                           levels = c("(Intercept)", 
                                                      "Stress Ante", 
                                                      "Stress Ante:Syllable 1.5", 
                                                      "Stress Ante:Syllable 2", 
                                                      "Syllable 1.5", 
                                                      "Syllable 2"))
combined_tidy_models$term <- factor(combined_tidy_models$term, levels = unique(combined_tidy_models$term))


# Plot the combined models
pen_anti_model <- ggplot(data = combined_tidy_models, aes(x = estimate, y = term, color = factor(looks))) +
  geom_point(position = position_dodge(width = 0.5)) + #adds order to legend
  geom_point(
    aes(x = estimate, y = term, color = factor(looks)), 
    position = position_dodge(width = 0.5),
    data = combined_tidy_models_legend)+
  geom_point(position = position_dodge(width = 0.5)) +  # Plot points from combined_tidy_models over fake ones
  ...
  labs(title = "", 
       x = "", 
       y = "", 
       color = "Looks to Visual Stimuli") +
  theme_minimal()+
  geom_vline(xintercept = 0, color = "black")+
  coord_flip()+
  scale_color_manual(
    values = c("Target Looks" = "#c41230", 
               "Competitor Looks" = "#007bc0", 
               "Distractor 1" = "#000000", 
               "Distractor 2" = "#A7A7A9"),
    labels = c("\nTarget\n", 
               "\nCompetitor\n", 
  ...
  theme(
    ...
    legend.position = "right", # Adjust the position to move the legend down
    legend.justification = c(0, 0.5),
    legend.background = element_rect(fill = "white", colour = "white")
  )+
  geom_text(
    aes(y = term, x = -0.5, label = Significance,color = factor(looks)), 
    hjust = 0, vjust = .5, angle = 90, , show.legend = FALSE,
    position= position_dodge(width = 0.5))+
  scale_y_discrete(expand = expansion(add = c(-.9, .7)))
...
```{r}
combined_plot_2 <- plot_grid(plot2,plot1, pen_anti_model, ncol = 1,rel_heights = c(2,2,2))+
  draw_label("Estimated Coefficients                                       Penultimate                              Antepenultimate ", x = 0.02, y = 0.5, angle = 90, size = 14)

combined_plot_2
ggsave("../visuals/pen_vs_anti_pen_id_combo.jpeg", plot = combined_plot_2, width = 6, height = 10, dpi = 300)
```
...
tar_longer<-comp_tar_longer%>%
  ...
  group_by(word_play,stress_play,syllable,participant.private.id)%>%
  summarize(tar_prop_o_looks=mean(values))%>%
  mutate(as.factor(syllable))%>%
  filter(syllable!=c(0,1))
  
levels(tar_longer$syllable)
levels(tar_longer$stress_play)
library(lmerTest)
tar_ana1<-lmerTest::lmer(tar_prop_o_looks~stress_play+(syllable|word_play)+(1|participant.private.id),data=tar_longer)
tar_ana2<-lmerTest::lmer(tar_prop_o_looks~stress_play+(1|word_play)+(1|participant.private.id),data=tar_longer)
...
acoustic_data<-acoustic_data%>%
 mutate(
    stress_play = ifelse(stress == "antipenultimate", "Antepenultimate", "Penultimate"),
    syllable = ifelse(vowel == "v1", "Vowel 1", "Vowel 2"),
    syllable=as.factor(syllable),
    word=tolower(word))%>%
  select(-X)

acoustic_data_longer<-acoustic_data%>%
  select(stress_play,syllable,mean_amplitude,mean_pitch,duration,mean_tilt,word)%>%
  pivot_longer(cols=c(mean_amplitude,mean_pitch,duration,mean_tilt),
               names_to = "type",
               values_to = "values")%>%
  mutate(base = case_when(
    str_starts(word, "c") ~ str_sub(word, 1, 4), # Remove last 3 characters if starts with 'c'
    TRUE ~ str_sub(word, 1, 3)                    # Remove last 4 characters otherwise
  ))

acoustic_data_longer$type <- factor(acoustic_data_longer$type,
                                    levels = c("mean_amplitude", "mean_pitch", 
                                               "duration", "mean_tilt"),
                                    labels = c("Amplitude (dB)", "Pitch (Hz)", 
                                               "Duration (ms)", "Spectral Tilt"))
acoustic_data_longer2<-acoustic_data_longer %>%
  group_by(type) %>%
  mutate(scaled_value = if (n() > 1) as.numeric(scale(values)) else values) %>%
  ungroup()
  
acoustic_data_longer2 <- acoustic_data_longer2 %>%
  mutate(custom_x = case_when(
    paste(syllable, type,sep=".") == "Vowel 1.Amplitude (dB)" ~ 1,
    paste(syllable, type,sep=".") == "Vowel 2.Amplitude (dB)" ~ 2,
    paste(syllable, type,sep=".") == "Vowel 1.Pitch (Hz)" ~ 4,
    paste(syllable, type,sep=".") == "Vowel 2.Pitch (Hz)" ~ 5, 
    paste(syllable, type,sep=".") == "Vowel 1.Duration (ms)" ~ 7,
    paste(syllable, type,sep=".") == "Vowel 2.Duration (ms)" ~ 8,
    paste(syllable, type,sep=".") == "Vowel 1.Spectral Tilt" ~ 10, 
    paste(syllable, type,sep=".") == "Vowel 2.Spectral Tilt" ~ 11))


plot_line <- acoustic_data_longer2 %>%
  ggplot(aes(x = custom_x, y = scaled_value, color = stress_play, group = word)) +
  geom_rect(aes(xmin = 2.01, xmax = 3.99, ymin = -Inf, ymax = Inf), fill = "white",color="white") +
  geom_rect(aes(xmin = 5.01, xmax = 6.99, ymin = -Inf, ymax = Inf), fill = "white",color="white") +
  geom_rect(aes(xmin = 8.01, xmax = 9.99, ymin = -Inf, ymax = Inf), fill = "white",color="white") +
  geom_rect(aes(xmin = -Inf, xmax = .99, ymin = -Inf, ymax = Inf), fill = "white",color="white") +
  geom_rect(aes(xmin = 11.01, xmax = Inf, ymin = -Inf, ymax = Inf), fill = "white",color="white") +
  geom_point(size = .2) +
  geom_line(aes(group = interaction(word, stress_play)), alpha = 0.8, size = .1) +
  facet_wrap(syllable ~ ., ncol = 1) +
  theme_minimal() +
  scale_color_manual(values = c("Antepenultimate" = "#E77500", "Penultimate" = "#000000")) +
  scale_x_continuous(
    breaks = c(1,2,4,5,7,8,10,11),
    labels = c("Vowel 1", "Vowel 2", "Vowel 1", "Vowel 2", "Vowel 1", "Vowel 2","Vowel 1", "Vowel 2"),
    minor_breaks = c(-1,0,1, 2,4,5,7,8,10,11),
    expand = expansion(mult = c(.075, .06))
  ) +
 theme(
    legend.position = "none",
  ...
  labs(title = "", y = "", color = "Stress Play")
plot_line
...
```{r}
# Assuming acoustic_data_longer2 is your input dataset
# Prepare the data for polygons
acoustic_data_longer3 <- acoustic_data_longer2 %>%
  arrange(syllable, base, stress_play, type) %>%
  group_by(syllable, base, stress_play) %>%
  mutate(type = as.factor(type)) %>%
  mutate(scaled_value = as.numeric(scaled_value)) %>%
  group_modify(~ {
    data <- .x
    if (nrow(data) > 0) {
      data <- bind_rows(data, data[1,]) # Repeat the first row at the end to close the polygon
    }
    data
  }) %>%
  ungroup() %>%
...
acoustic_data_longer3 <- acoustic_data_longer3 %>%
  group_by(syllable, base, stress_play) %>%
  mutate(
    angle = (row_number() - 1) * (360 / n()),
    x = sin(pi * angle / 180) * scaled_value,
    y = cos(pi * angle / 180) * scaled_value
  ) %>%
  ungroup()


# Custom coordinate system to avoid curved lines
...
# Circular plot with paths and custom labels
circular_plot_path <- ggplot(acoustic_data_longer3, aes(x = id, y = scaled_value, color = stress_play, group = interaction(syllable, base, stress_play))) +
  geom_path(size = .1, alpha = .5) +
  geom_point(size = .2) +
  facet_wrap(~ syllable,ncol=1) +
  geom_text(aes(x = id, y = max(acoustic_data_longer3$scaled_value) * 1.1, label = type), size = 3, inherit.aes = FALSE) +
  coord_straightpolar(start = 0) +
  theme_minimal() +
  scale_color_manual(values = c("Antepenultimate" = "#E77500", "Penultimate" = "#000000")) +
  theme(
    legend.position = "none",
...
circular_plot_path
# Circular plot with polygons
circular_plot_polygon <- ggplot(acoustic_data_longer3, aes(x = angle, y = scaled_value,color=stress_play, group = interaction(syllable, base, stress_play))) +
  geom_polygon(aes(fill = stress_play,color=stress_play), alpha = 0.01, size = 0.01) +
  facet_wrap(stress_play~ syllable) +
  coord_straightpolar(start = 0) +
  theme_minimal() +
  scale_fill_manual(values = c("Antepenultimate" = "#E77500", "Penultimate" = "#000000")) +
  scale_color_manual(values = c("Antepenultimate" = "#E77500", "Penultimate" = "#000000")) +
  theme(
...
```{r}
library(gghalves)
raw_acoustics<-acoustic_data_longer %>%
  ggplot() +
  # Add a white rectangle covering the middle part of the x-axis
  geom_rect(aes(xmin = 1, xmax = 2, ymin = -Inf, ymax = Inf), fill = "white") +
  geom_point(aes(x = syllable, y = values, color = stress_play), shape = 95, alpha = 0.8, size = 3, position = position_jitterdodge(jitter.width = 0.0, dodge.width = 0.0)) +
  geom_half_violin(aes(x = syllable, y = values, color = stress_play), 
                   alpha = 0.5, side = "l", position = position_nudge(x = -0.), 
                   data = acoustic_data_longer %>% filter(stress_play == "Penultimate")) +
  geom_half_violin(aes(x = syllable, y = values, color = stress_play), 
                   alpha = 0.5, side = "r", position = position_nudge(x = 0.), 
                   data = acoustic_data_longer %>% filter(stress_play == "Antepenultimate")) +
  geom_line(aes(x = syllable, y = values, group = interaction(base, stress_play, type, type), color = stress_play,se=FALSE), 
            alpha = 0.5, size = 0.1) +
  geom_smooth(aes(x = syllable,  y = values, color = stress_play, 
                  group = interaction(stress_play, type),fill=stress_play),alpha=.3, method = "lm",size=1,show.legend = FALSE) +
  geom_smooth(aes(x = syllable,  y = values, 
                  group = interaction(stress_play, type),fill=stress_play),alpha=.3, color = "white",method = "lm",size=1,linetype="dotted",show.legend = FALSE) +
...
combined_plot <- plot_grid(
  raw_acoustics + theme(plot.margin = unit(c(0, 0, 0, 0), "cm")),
  plot_line + theme(plot.margin = unit(c(-1, 0, 0, 0), "cm")),
  nrow = 2,  # Two rows
  rel_heights = c(1, 1))
combined_plot

ggsave("../visuals/raw_acoustics.jpeg", plot = raw_acoustics , width = 8, height = 6, dpi = 300) 
ggsave("../visuals/raw_acoustics_combined.jpeg", plot = combined_plot , width = 10, height = 8, dpi = 300) 
#"          Vowel 1                Vowel 2                                     Vowel 1                Vowel 2"
```
...
```
#analysis 3 target fixation analysis (interaction model not in paper)
```{r}
acoustic_data <- read.csv(file.path(target_dir, "acoustic_data.csv"))
...
#                +(1|word)+(1|participant.private.id),family=binomial, data= ap_data_1)

#mod_full_2<-glmer(target_looks~(mean_pitch_scaled+mean_amplitude_scaled+mean_tilt_scaled+duration_scaled)^2
#                +(1|word)+(1|participant.private.id),family=binomial, data= pp_data_1)
...
#                +(1|word)+(1|participant.private.id),family=binomial, data= ap_data_3)

#mod_full_2_3<-glmer(target_looks~(mean_pitch_scaled+mean_amplitude_scaled+mean_tilt_scaled+duration_scaled)^2
#                +(1|word)+(1|participant.private.id),family=binomial, data= pp_data_3)
...
analysis_3_plot <- ggplot(tidy_combined, aes(x = estimate, y = term, color = model)) +
  ...
  labs(title = "", x = "Estimate", y = "",color = "Model Type") +
  theme_minimal() +
  geom_vline(xintercept = 0, linetype = "dashed") +
  facet_grid(. ~ syllable, labeller = labeller(syllable = custom_labels),scales="free") +
  geom_text(aes(label = Significance, x = 1.5), vjust = 0.5, position = position_dodge(width = 0.5),show.legend = FALSE) +
  scale_color_manual(values = c("Antepenultimate" = "#E77500", "Penultimate" = "#000000")) +
  theme(legend.position = "none")+
  scale_x_continuous(limits = c(-3.5, 1.75))
# Print the plot
analysis_3_plot 

ggsave("../visuals/analysis_3_plot.jpeg", plot = analysis_3_plot , width = 8, height = 5, dpi = 300)
````
3.1.4 without interactions:
```{r}
# Function to perform backward elimination
backward_elimination <- function(model, data) {
  current_model <- model
...
```{r}
mod_full_ni<-glmer(target_looks~(mean_pitch_scaled+mean_amplitude_scaled+mean_tilt_scaled+duration_scaled)
                +(1|word)+(1|participant.private.id),family=binomial, data= ap_data_1)

mod_full_2_ni<-glmer(target_looks~(mean_pitch_scaled+mean_amplitude_scaled+mean_tilt_scaled+duration_scaled)
                +(1|word)+(1|participant.private.id),family=binomial, data= pp_data_1)

mod_full_1_3_ni<-glmer(target_looks~(mean_pitch_scaled+mean_amplitude_scaled+mean_tilt_scaled+duration_scaled)
                +(1|word)+(1|participant.private.id),family=binomial, data= ap_data_3)

mod_full_2_3_ni<-glmer(target_looks~(mean_pitch_scaled+mean_amplitude_scaled+mean_tilt_scaled+duration_scaled)
                +(1|word)+(1|participant.private.id),family=binomial, data= pp_data_3)

summary(mod_full_ni)
summary(mod_full_2_ni)

summary(mod_full_1_3_ni)
summary(mod_full_2_3_ni)

simplified_ap_mod_1_ni <- backward_elimination(mod_full_ni, ap_data_1)
simplified_pp_mod_1_ni <- backward_elimination(mod_full_2_ni, pp_data_1)
simplified_ap_mod_3_ni <- backward_elimination(mod_full_1_3_ni, ap_data_3)
simplified_pp_mod_3_ni <- backward_elimination(mod_full_2_3_ni, pp_data_3)


summary(simplified_ap_mod_1_ni)
summary(simplified_pp_mod_1_ni)
summary(simplified_ap_mod_3_ni)
summary(simplified_pp_mod_3_ni)
```
...
```{r}
# Extract summaries for each model
tidy_full_ni <- tidy(simplified_ap_mod_1_ni, effects = "fixed")
tidy_full_2_ni <- tidy(simplified_pp_mod_1_ni, effects = "fixed")
tidy_full_1_3_ni <- tidy(simplified_ap_mod_3_ni, effects = "fixed")
tidy_full_2_3_ni <- tidy(simplified_pp_mod_3_ni, effects = "fixed")

# Add a model identifier
tidy_full_ni$model <- "Antepenultimate"
tidy_full_2_ni$model <- "Penultimate"
tidy_full_1_3_ni$model <- "Antepenultimate"
tidy_full_2_3_ni$model <- "Penultimate"

tidy_full_ni$syllable <- 1
tidy_full_2_ni$syllable <- 1
tidy_full_1_3_ni$syllable <- 3
tidy_full_2_3_ni$syllable <- 3

# Combine the summaries into one data frame
tidy_combined_ni <- bind_rows(tidy_full_ni, tidy_full_2_ni, tidy_full_1_3_ni, tidy_full_2_3_ni)

# Add a significance marker
tidy_combined_ni <- tidy_combined_ni %>%
  mutate(Significance = case_when(
    p.value < 0.001 ~ "***",
    p.value < 0.01 ~ "**",
    p.value < 0.05 ~ "*",
...
ordered_terms_order_ni <- tidy_combined_ni %>%
  arrange(interaction_label) %>%
  pull(term) %>%
  unique()
...
# Set factor levels for term
tidy_combined_ni$term <- factor(tidy_combined_ni$term, levels = ordered_terms_order_ni)

custom_labels <- c(`1` = "Syllable 1", `3` = "Syllable 2")
# Plot for all models
analysis_3_plot_ni <- ggplot(tidy_combined_ni, aes(x = estimate, y = term, color = model)) +
  geom_point(position = position_dodge(width = 0.5), size = 3) +
  geom_errorbarh(aes(xmin = estimate - 1.96 * std.error, xmax = estimate + 1.96 * std.error), 
                 height = 0, position = position_dodge(width = 0.5), size = 1) +
  labs(title = "", x = "Estimate", y = "",color = "Model Type") +
  ...
  geom_vline(xintercept = 0, linetype = "dashed") +
  facet_grid(. ~ syllable, labeller = labeller(syllable = custom_labels),scales="free") +
  geom_text(aes(label = Significance, x = .5), vjust = 0.5, position = position_dodge(width = 0.5),show.legend = FALSE) +
  scale_color_manual(values = c("Antepenultimate" = "#E77500", "Penultimate" = "#000000")) +
  theme(legend.position = "bottom")+
...
ggsave("../visuals/analysis_3_plot_ni.jpeg", plot = analysis_3_plot_ni , width = 8, height = 5, dpi = 300)
```
```{r}

analysis_3_plot_ni <- analysis_3_plot_ni + 
  ggtitle("Primary Model\n")+
  theme(plot.title = element_text(hjust = 0.5)) 

analysis_3_plot <- analysis_3_plot + 
  ggtitle(expression(atop(Interaction~Model, "(see"~italic("supplemental materials")~")")))

combined_plot_o_n <- plot_grid(analysis_3_plot_ni, analysis_3_plot, ncol = 2)
combined_plot_o_n

ggsave("../visuals/combined_plot_o_n.jpeg", plot = combined_plot_o_n , width = 8, height = 5, dpi = 300)
```
...
#This analysis over fixation is only for testing the method. The actual one is below
```{r}
...
fit_models <- function(data, stress_filter) {
  ...
  x <- model.matrix(values ~ syllable * (dprime_battery_up_dur + dprime_battery_up_ptich + dprime_battery_up_risetime + dprime_battery_up_formants + lextale_score + autism_score + italian_lextale_score+mean_working_memory), data = data_filtered)[, -1]
...
```
Individual differences analysis---- This is the analysis in the paper
...
long_data <- measures %>%
  ...
  mutate(measure = str_replace_all(measure, "italian lextale score", "LexITA")) %>%
  mutate(measure = str_replace_all(measure, "lextale score", "LexTALE")) %>%
  mutate(measure = str_replace_all(measure, "scaled", "")) %>%
  mutate(measure = str_replace_all(measure, "dprime", "")) %>%
  mutate(measure = str_replace_all(measure, "autism score", "AQ")) %>%
...
plot<-ggplot(long_data, aes(x = measure, y = value)) +
  ...
  geom_half_boxplot(width = 0.15, outlier.shape = NA, fill = NA, color = "black", side = "r") +
  geom_point(shape = 95, color = "black", size = 10, alpha = 0.3, position = position_nudge(x = -0.1)) +
  labs(title = "",
       x = "Task Measures",
       y = "Centered Values") +
  theme_minimal() +
  ...
  geom_line(aes(color = factor(participant.private.id),group=participant.private.id), size = .1,alpha=1)
...
fit_models_acoustic_id <- function(data) {
  ...
  # Step 1: Extract acoustic variables directly from the 'names' dataframe
  acoustic_vars <- unique(names$term)

  # Remove intercept or any unwanted variables from acoustic_vars
  acoustic_vars <- acoustic_vars[acoustic_vars != "(Intercept)"]

  # Define relevant pairings of sensitivity measures and acoustic variables
  sensitivity_mapping <- list(
    "mean_pitch_scaled" = "dprime_battery_up_ptich",
    "duration_scaled" = "dprime_battery_up_dur",
    "mean_amplitude_scaled" = "dprime_battery_up_risetime",  
    "mean_tilt_scaled" = "dprime_battery_up_formants"
  )

  # General sensitivity-related measures that could pair with any acoustic variable
  general_sensitivity_vars <- c("lextale_score", 
                                "autism_score", 
                                "italian_lextale_score", 
                                "mean_working_memory")

  # Initialize list to store interaction terms
  interaction_terms <- list()

  # 1. Create two-way interactions between relevant acoustic variables and their corresponding sensitivity measures
  for (acoustic_var in acoustic_vars) {
    if (acoustic_var %in% names(sensitivity_mapping)) {
      # Add two-way interaction between acoustic variable and its relevant sensitivity measure
      interaction_terms[[length(interaction_terms) + 1]] <- paste(acoustic_var, sensitivity_mapping[[acoustic_var]], sep = ":")
    }
    # Create two-way interactions with general sensitivity measures
    for (general_var in general_sensitivity_vars) {
      interaction_terms[[length(interaction_terms) + 1]] <- paste(acoustic_var, general_var, sep = ":")
    }
  }

  # 2. Create three-way interactions only for valid acoustic-sensitivity combinations
  acoustic_interactions <- combn(acoustic_vars, 2, simplify = FALSE)
  for (acoustic_interaction in acoustic_interactions) {
    interaction_term <- paste(acoustic_interaction, collapse = ":")

    # For each pair of acoustic variables, we check for relevant sensitivity measures
    for (acoustic_var in acoustic_interaction) {
      sensitivity_measure <- sensitivity_mapping[[acoustic_var]]
      if (!is.null(sensitivity_measure)) {
        # Only add the three-way interaction if the sensitivity measure matches
        interaction_terms[[length(interaction_terms) + 1]] <- 
          paste(interaction_term, sensitivity_measure, sep = ":")
      }
    }
  }
  # Combine acoustic vars and interaction terms for the maximal model
  all_vars <- c(acoustic_vars, unlist(interaction_terms))
  # Create the maximal model formula string
  formula_str <- paste("target_looks ~ autism_score + 
                       italian_lextale_score + 
                       lextale_score + 
                       mean_working_memory +",
                       paste(sensitivity_mapping, collapse = " + "), "*", "(",
                       paste(acoustic_vars, collapse = "+"), ")")

  # Convert the formula string into an actual formula object
  formula <- as.formula(formula_str)

  # Step 2: Create the design matrix and response vector based on the maximal model
  x <- model.matrix(formula, data = data_filtered)[, -1]  # Exclude intercept
  y <- data_filtered$target_looks
  
  # Step 3: Perform LASSO regression on the design matrix and response vector
  lambda_search_space <- 10^seq(10, -2, length = 100)
  ...
  coef_plot <- ggplot(coef_df, aes(x = Predictor, y = s1)) +
    ...
    coord_flip() +
    labs(title = 
           paste("LASSO Regression Coefficients at Best Lambda (", 
                 round(best_lambda, 4), ")", sep = ""),
         x = "Predictor",
         y = "Coefficient Value") +
    theme_minimal()
  print(coef_plot)

  # Step 4: Select significant variables based on LASSO coefficients and clean them
  significant_vars <- coef_df %>%
    filter(s1 != 0) %>%
    pull(Predictor)
  ...
  # Step 5: Prepare final formula for GLMM using significant variables from LASSO
  formula_str <- paste("target_looks ~", 
                       paste(significant_vars, collapse = " + "), "+ (1|word)")
  formula <- as.formula(formula_str)

  # Step 6: Fit GLMM with selected variables
  model_selected <- glmer(formula, data = data_filtered, family = "binomial")
...
fit_models_acoustic_id <- function(data) {
  ...
  # Create the design matrix and response vector
  x <- model.matrix(target_looks ~ autism_score + 
                                      italian_lextale_score + 
  ...
  # Prepare formula for GLMM using significant variables
  significant_vars <- significant_vars[significant_vars != "(Intercept)"] # Remove intercept if present
  formula_str <- paste("target_looks ~", paste(significant_vars, collapse = " + "), "+ (1|word)+(1|participant.private.id)")
...
id_vars <- c("dprime_battery_up_dur", "dprime_battery_up_ptich", "dprime_battery_up_risetime", "dprime_battery_up_formants", "lextale_score", "autism_score", "italian_lextale_score", "mean_working_memory")
...
all_coefs <- all_coefs %>%
  ...
  mutate(term = str_replace_all(term, "dprime battery up", "(battery d')")) %>%
  mutate(term = str_replace_all(term, "mean", "")) %>%
  mutate(term = str_replace_all(term, "italian lextale score", "lexITA")) %>%
  mutate(term = str_replace_all(term, "lextale score", "lexTALE")) %>%
  mutate(term = str_replace_all(term, "scaled", "")) %>%
  mutate(term = str_replace_all(term, "dprime", "")) %>%
  mutate(term = str_replace_all(term, "autism score", "AQ")) %>%
  mutate(term = str_replace_all(term, "tilt", "spectral tilt")) %>%
  mutate(term = str_replace_all(term, "(battery d') dur", "(battery d') duration"))
...
create_coef_plot <- function(coef_df, model_label, remove_y_labels = FALSE) {
  plot <- ggplot(coef_df, aes(x = Estimate, y = term, color = model_color)) +
    ...
    geom_text(aes(label = significance, x = 2.5, color = model_color), vjust = 0.5, position = position_dodge(width = 0.5),show.legend = FALSE) +
    labs(title = "",
         x = "Estimate",
    ...
    facet_wrap(.~ syllable,scales="free_x")+
  scale_color_manual(values = c("#E77500","#000000")) +
  theme(legend.position = "bottom")
  return(plot)
}
...
# bibtex format
r_citation <- citation()
bibtex_citation <- toBibtex(r_citation)
cat(bibtex_citation)
