...
freq_data <- read.csv(file.path(target_dir, "Word_Data.csv"))
...
pen_anti_model <- ggplot(data = combined_tidy_models, aes(x = estimate, y = term, color = factor(looks))) +
  ...
  geom_point(position = position_dodge(width = 0.5)) +  # Plot points from combined_tidy_models over fake ones that are there just for legend
...
#test for meaningful nature of frequency
```{r}
freq_data_clean<-freq_data%>%
  mutate(word_play = tolower(Word))
...
tar_longer<-comp_tar_longer%>%
  ...
  mutate(as.factor(syllable))%>%
  filter(syllable!=c(0,1))%>%
  left_join(freq_data_clean)
  
levels(tar_longer$syllable)
levels(tar_longer$stress_play)
library(lmerTest)
tar_ana1<-lmerTest::lmer(tar_prop_o_looks~stress_play*Frequency+(syllable|word_play)+(1|participant.private.id),data=tar_longer)
tar_ana2<-lmerTest::lmer(tar_prop_o_looks~stress_play*Frequency+(1|word_play)+(1|participant.private.id),data=tar_longer)
...
acoustic_data_longer<-acoustic_data%>%
  ...
  mutate(base = case_when(
    str_starts(word, "c") ~ str_sub(word, 1, 4),
    TRUE ~ str_sub(word, 1, 3)                    
  ))
...
```
3.1.4 without interactions (primary models):
```{r}
# Function to perform backward elimination
backward_elimination <- function(model, data) {
  current_model <- model
...
analysis_3_plot_ni <- analysis_3_plot_ni + 
  ggtitle("Primary Models\n")+
  theme(plot.title = element_text(hjust = 0.5)) 

analysis_3_plot <- analysis_3_plot + 
  ggtitle(expression(atop(Interaction~Models, "(see"~italic("supplemental materials")~")")))

combined_plot_o_n <- plot_grid(analysis_3_plot_ni, analysis_3_plot, ncol = 2)
combined_plot_o_n

#ggsave("../visuals/combined_plot_o_n.jpeg", plot = combined_plot_o_n , width = 8, height = 5, dpi = 600)
```
...
#This analysis over fixation is only for testing the method. That is- here we are showing how the previous acoustic models could be done with LASSO for future reference if future research would like to use the method without the complexity of the indifividual differences. The actual one for individual differences (in the paper extension model) is further down
```{r}
...
original_summary <- id_data %>%
  na.omit()%>%
  select(participant.private.id, starts_with("dprime_battery_up_"), 
         mean_working_memory, lextale_score, italian_lextale_score, autism_score) %>%
  pivot_longer(cols = -participant.private.id, 
               names_to = "task", 
               values_to = "values") %>%
  group_by(task) %>%
  summarize(min = min(values),
            ...
            max = max(values))%>%
  pivot_longer(cols=!task,names_to = "category",values_to = "original_values")

# Scaled summary for min, median, and max
scaled_summary <- measures %>%
  na.omit()%>%
  select(participant.private.id, contains("scaled_")) %>%
  pivot_longer(cols = -participant.private.id, 
               names_to = "task", 
               values_to = "scaled_values") %>%
  mutate(task = gsub("scaled_", "", task)) %>%
  group_by(task) %>%
  summarize(min = min(scaled_values),
            median = median(scaled_values),
            max = max(scaled_values))%>%
  pivot_longer(cols=!task,names_to = "category",values_to = "scaled_values")

# Combine original and scaled summaries and pivot longer to create 4-column output
final_summary <- original_summary %>%
  left_join(scaled_summary)%>%
  mutate(old_term=task)%>%
  left_join(unique(long_data%>%select(old_term,measure)))%>%
  mutate(original_values=round(original_values,2))

density_data <- final_summary %>%
  group_by(measure) %>%
  do({
    dens <- density(.$scaled_values)
    scaled_values_continuous <- dens$x
    density <- dens$y
    
    # Calculate cumulative density to find the median
    cumulative_density <- cumsum(density) / sum(density)
    
    # Find index where cumulative density is closest to 0.5 (the median)
    median_index <- which.min(abs(cumulative_density - 0.5))
    
    data.frame(
      scaled_values_continuous = scaled_values_continuous,
      density = density,
      cumulative_density = cumulative_density,
      min_scaled = min(scaled_values_continuous),
      max_scaled = max(scaled_values_continuous),
      median_scaled = scaled_values_continuous[median_index],  #Median value using cumulative density
      min_density = density[which.min(scaled_values_continuous)],
      max_density = density[which.max(scaled_values_continuous)],
      median_density = density[median_index]  # Corresponding density for the 50th percentile
    )
  }) %>%
  select(measure, min_density, median_density, max_density) %>%
  rename_with(~ gsub("_density", "", .x)) %>%
  pivot_longer(cols = !measure, names_to = "category", values_to = "density")%>%
  unique()

# Perform nearest join based on scaled_values
final_summary <- final_summary %>%
  left_join(density_data)%>%
    mutate(x_numeric = as.numeric(as.factor(measure)),
           nudged_x = as.numeric(as.factor(measure)) - (density*2)-.05) 


plot<-ggplot(long_data, aes(x = measure, y = value)) +
  geom_line(aes(group=interaction(participant.private.id),color=interaction(participant.private.id)),size=.07,alpha=1)+
  geom_half_boxplot(width = 0.15, outlier.shape = NA, fill = "black", color = "white", side = "r",alpha=.5) +
  geom_half_violin(fill = "white",color="black",trim = FALSE, color = "black", side = "l",alpha=.5)+
  #geom_point(shape = 95, color = "black", size = 5, alpha = 0.5, position = position_nudge(x = -0.1)) +
  geom_label(aes(x = nudged_x, 
                 y = scaled_values,
                 label = sprintf("%.2f", original_values)),
             data = final_summary,
             fill = "white",    # Background color of the box
             color = NA,   # Text color inside the label
             label.size = 0.2,  # Thickness of the border of the box
             label.padding = unit(0.15, "lines"),alpha=.8) +  # Padding inside the box
  geom_text(aes(x = nudged_x, 
                y = scaled_values,
                label = sprintf("%.2f", original_values)),
            data=final_summary,
            color="#333333",
...
#lasso model for extension function--- this is the one in the paper.
```{r}
control_params <- glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000), tolPwrss = 1e-3)
...
fit_models_acoustic_id <- function(data) {
  ...
  model_selected <- glmer(formula, data = data_filtered, family = "binomial",control = control_params)
...
ggsave("../visuals/extended_analysis.jpeg", plot = plot , width = 8, height = 6, dpi = 600)
```

only for citing purposes
```{r}
r_version <- R.version
