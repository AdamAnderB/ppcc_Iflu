...
combined_tidy_models <- combined_tidy_models %>%
  ...
  arrange(syllable, looks,syllable) %>%
  mutate(
...
pen_anti_model <- ggplot(data = combined_tidy_models, aes(x = estimate, y = term, color = factor(looks))) +
  ...
  geom_point(position = position_dodge(width = 0.5)) +  # Plot points from combined_tidy_models
...
combined_plot_o_n <- plot_grid(analysis_3_plot_ni, analysis_3_plot, labels = c("Simple Model", "Interaction Model"), ncol = 2)
combined_plot_o_n
...
plot<-ggplot(long_data, aes(x = measure, y = value)) +
  ...
  geom_point(shape = 95, color = "#1F4C4C", size = 10, alpha = 0.3, position = position_nudge(x = -0.1)) +
  labs(title = "",
       x = "Task Measures",
       y = "Centered Values") +
  theme_minimal() +
...
fit_models_acoustic_id <- function(data) {
  ...
  # Assuming 'names' is a global dataframe or exists within the environment
  ...
  # Define sensitivity measures for the relevant acoustic variables
  sensitivity_measures <- "(dprime_battery_up_dur + dprime_battery_up_ptich + dprime_battery_up_risetime + dprime_battery_up_formants)"

  # Dynamically build the acoustic interaction terms
  acoustic_interactions <- paste(acoustic_vars, collapse = "+")

  # Create the maximal model formula string
  formula_str <- paste("target_looks ~ autism_score + italian_lextale_score + lextale_score + mean_working_memory +",
                       sensitivity_measures, "*", "(", acoustic_interactions, ")")
...
fit_models_acoustic_id <- function(data) {
  ...
  # Create the design matrix and response vector
  x <- model.matrix(target_looks ~ 
    # Main effects (General Sensitivity Measures)
    autism_score + italian_lextale_score + lextale_score + mean_working_memory +
    
    # Main effects (Sensitivity Measures)
    dprime_battery_up_dur + dprime_battery_up_ptich + dprime_battery_up_risetime + dprime_battery_up_formants +
    
    # Main effects (Acoustic Measures)
    mean_pitch_scaled + duration_scaled + mean_amplitude_scaled + mean_tilt_scaled +
    
    # Two-way interactions (Acoustic^2)
    mean_pitch_scaled:duration_scaled + mean_pitch_scaled:mean_amplitude_scaled + mean_pitch_scaled:mean_tilt_scaled + 
    duration_scaled:mean_amplitude_scaled + duration_scaled:mean_tilt_scaled + 
    mean_amplitude_scaled:mean_tilt_scaled +
    
    # Three-way interactions (Acoustic^2 with Sensitivity Measures)
    mean_pitch_scaled:duration_scaled:dprime_battery_up_dur + mean_pitch_scaled:duration_scaled:dprime_battery_up_ptich +
    mean_pitch_scaled:duration_scaled:dprime_battery_up_risetime + mean_pitch_scaled:duration_scaled:dprime_battery_up_formants +
    
    mean_pitch_scaled:mean_amplitude_scaled:dprime_battery_up_dur + mean_pitch_scaled:mean_amplitude_scaled:dprime_battery_up_ptich +
    mean_pitch_scaled:mean_amplitude_scaled:dprime_battery_up_risetime + mean_pitch_scaled:mean_amplitude_scaled:dprime_battery_up_formants +
    
    mean_pitch_scaled:mean_tilt_scaled:dprime_battery_up_dur + mean_pitch_scaled:mean_tilt_scaled:dprime_battery_up_ptich +
    mean_pitch_scaled:mean_tilt_scaled:dprime_battery_up_risetime + mean_pitch_scaled:mean_tilt_scaled:dprime_battery_up_formants +
    
    duration_scaled:mean_amplitude_scaled:dprime_battery_up_dur + duration_scaled:mean_amplitude_scaled:dprime_battery_up_ptich +
    duration_scaled:mean_amplitude_scaled:dprime_battery_up_risetime + duration_scaled:mean_amplitude_scaled:dprime_battery_up_formants +
    
    duration_scaled:mean_tilt_scaled:dprime_battery_up_dur + duration_scaled:mean_tilt_scaled:dprime_battery_up_ptich +
    duration_scaled:mean_tilt_scaled:dprime_battery_up_risetime + duration_scaled:mean_tilt_scaled:dprime_battery_up_formants +
    
    mean_amplitude_scaled:mean_tilt_scaled:dprime_battery_up_dur + mean_amplitude_scaled:mean_tilt_scaled:dprime_battery_up_ptich +
    mean_amplitude_scaled:mean_tilt_scaled:dprime_battery_up_risetime + mean_amplitude_scaled:mean_tilt_scaled:dprime_battery_up_formants,
    
    data = data_filtered)[, -1]  # Exclude intercept
