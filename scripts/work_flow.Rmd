...
plot1<-ets_exp1_all_agg %>%
  ...
  scale_color_manual(
    ...
    labels = c("Target", "Competitor", "Distractor", "Distractor")
  )
...
subset_fit <- leaps::regsubsets(formula_interactions, data = full_data_exp1, nbest = 1, nvmax = 10, force.in = force_in_vector,really.big=T)
...
subset_fit2 <- leaps::regsubsets(formula_interactions2, data = full_data_exp2, nbest = 1, nvmax = 10, force.in = force_in_vector2,really.big=T)
...
formula_str2 <- gsub("Penultimate", "", formula_str2, perl = TRUE)
...
# Plot
ggplot(combined_df, aes(x = Predictor, y = Estimate, ymin = CI_Lower, ymax = CI_Upper, color = model)) +
  geom_pointrange(position = position_dodge(width = 0.2)) +  # Adjust 'width' to change the dodge amount
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x = "Predictor", y = "Estimate", title = "Comparison of Coefficient Estimates with Confidence Intervals") +
  scale_color_manual(values = c("competition" = "blue", "target looks" = "red"))+coord_flip()
```
