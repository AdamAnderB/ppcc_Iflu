...
tidy_model <- tidy(comp_tar_model, effects = "fixed")%>%
  mutate(
    main = case_when(
      str_detect(term, ":") ~ 1,
      TRUE ~ 0))%>%
...
pen_anti_model<-ggplot(tidy_model, aes(x = estimate, y = row_number,color=factor(looks))) +
  ...
  annotate("rect", xmin = -2.7, xmax = -1.45, ymin = -1.5, ymax = 32.5, fill = "white",color="grey") +
  geom_text(aes(y = row_number,x=-1.5, label = term, color = factor(looks)),hjust = 1, size = 4,angle=40)+
  xlim(-2.7, 1)+
  coord_flip()+
  ...
  geom_text(aes(y = 1,x=-2.5, label = "Terms"),color = "black",hjust = 1, size = 5,angle=0)
...
subset_fit <- regsubsets(formula_interactions, data = full_data_exp1, nbest = 1, nvmax = 10, force.in = force_in_vector,really.big=T)
