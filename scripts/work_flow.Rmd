...
analysis_3_plot <- ggplot(tidy_combined, aes(x = estimate, y = term, color = model)) +
  ...
  labs(title = "Model Comparison", x = "Estimate", y = "Term") +
  theme_minimal() +
...
plot<-ggplot(long_data, aes(x = measure, y = value)) +
  ...
  geom_point(shape = 95, color = "darkgreen", size = 3, alpha = 0.6) + # Vertical tick marks
  labs(title = "Half Boxplot, Half Violin Plot of Task Measures",
       x = "Task Measures",
       y = "Centered Value") +
  theme_minimal() +
...
result_ap_data_3_coefs$syllable <- "syllable 3"
...
result_pp_data_3_coefs$syllable <- "syllable 3"
...
full_data_clean <- full_data %>%
  select(stress_play, syllable, mean_pitch, mean_amplitude, duration, mean_tilt)%>%
  unique()%>%
  filter(syllable != 2)%>%
...
# Generate PDP for 'mean_pitch'
# Generate PDP for 'mean_pitch'
pdp_duration <- pdp::partial(rf_model, pred.var = "duration", train = data_standardized)
plot_duration <- autoplot(pdp_duration)

# Generate PDP for 'mean_pitch'
pdp_mean_pitch <- pdp::partial(rf_model, pred.var = "mean_pitch", train = data_standardized)
plot_mean_pitch <- autoplot(pdp_mean_pitch)

# Generate PDP for 'mean_tilt'
pdp_mean_tilt <- pdp::partial(rf_model, pred.var = "mean_tilt", train = data_standardized)
plot_mean_tilt <- autoplot(pdp_mean_tilt)

# Arrange plots side by side
grid.arrange(plot_duration, plot_mean_pitch, plot_mean_tilt, ncol = 3)
```
